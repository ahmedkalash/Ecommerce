                        ...
                        <tr>
                            ...
                            <td>
                                <div class="size-64px rounded-2 overflow-hidden">
                                    <img class="h-100 img-fit" src="{{ get_file_by_id($dynamic_popup->banner) }}"
                                         alt="">
                                </div>
                            </td>
                            ...
                            <td>
                                <label class="aiz-switch aiz-switch-primary mb-0">
                                    ...
                                    value="{{ $dynamic_popup->id }}" id="trigger_alert_{{ $dynamic_popup->id }}"
                                        type="checkbox" @if($dynamic_popup->status == 1) checked @endif
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                        @can('edit_dynamic_popups')
                                            <a class="dropdown-item"
                                               href="{{route('dynamic-popups.edit', $dynamic_popup->id)}}">
                                                {{translate('Edit')}}
                                            </a>
                                        @endcan
                                        @if($dynamic_popup->id != 1 && $dynamic_popup->id != 100)
                                            @can('delete_dynamic_popups')
                                                <a class="dropdown-item confirm-delete" href="javascript:void(0)"
                                                   data-href="{{route('dynamic-popups.destroy', $dynamic_popup->id)}}">
                                                    {{translate('Delete')}}
                                                </a>
                                            @endcan
                                        @endif
            ...
            <div class="modal-content p-2rem">
                <div class="modal-body text-center">
                    ...
                    <a href="javascript:void(0)" id="trigger_btn" data-value="" data-status="" data-clicked=""
                       ...
                       onclick="update_dynamic_popup_status()"></a>
                </div>
            </div>
        </div>
    </div><!-- /.modal -->
...
@section('script')
    <script type="text/javascript">
        ...
        function update_dynamic_popup_status(el) {
            ...
            $.post('{{ route('dynamic-popups.update-status') }}', {
                _token: '{{ csrf_token() }}',
                id: id,
                status: status
            }, function (data) {
