<!-- categories -->
<div class="aiz-carousel dashboard-box-carousel half-outside-arrow" data-items="6.5" data-xl-items="5"
     data-lg-items="4.5"
    ...
    @foreach ($top_categories_products as $key => $top_categories_product)
        <div class="carousel-box topproducts top_category_products @if ($key == $top_categories2[0]) active @endif"
             onclick="top_category_products({{ $key }}, this)">
            <div class="size-80px border border-dashed rounded-2 overflow-hidden p-1">
                <div class="h-100 rounded-2 overflow-hidden d-flex align-items-center">
                    <img src="{{ get_file_by_id($top_categories_product[0]->cover_image) }}"
