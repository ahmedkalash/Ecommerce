...
<div class="aiz-carousel dashboard-box-carousel half-outside-arrow" data-items="6.5" data-xl-items="4.5"
    ...
    @foreach ($new_top_sellers as $key => $top_sellers_product)
        <div class="carousel-box top_sellers_products @if ($key == 0) active @endif"
             onclick="top_sellers_products({{ $top_sellers_product->shop_id }}, this)">
            <div class="size-80px border border-dashed rounded-2 overflow-hidden p-1">
                <div class="h-100 rounded-2 overflow-hidden d-flex align-items-center">
                    <img src="{{ get_file_by_id($top_sellers_product->logo) }}" alt="{{ translate('sellers') }}"
...
<div class="position-relative">
    @foreach($new_top_sellers as $key => $top_sellers_product)
        <div
            class="top_sellers_product_table top-products-table table-responsive c-scrollbar-light @if ($key == 0) show @endif"
            ...
            <table class="table dashboard-table mb-0">
                ...
                @foreach($top_sellers_product->products as $row)
                    @php
                        $product_img = null;
                        if($row->thumbnail){
                            $product_img = ($row->thumbnail->external_link == null) ? get_file_by_path($row->thumbnail->file_name) : $row->thumbnail->external_link;
                        }
                        ...
                        <td class="pl-0" style="vertical-align: middle">
                            <div class="d-flex align-items-center">
                                ...
                                <a href="{{ $product_url }}" target="_blank"
                                   class="d-block text-soft-dark fw-400 hov-text-primary ml-2 fs-13"
                                   title="{{ $row->name }}">
