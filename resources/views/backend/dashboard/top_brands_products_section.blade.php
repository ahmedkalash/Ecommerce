...
<div class="aiz-carousel dashboard-box-carousel half-outside-arrow" data-items="{{ $data_items }}"
     ...
     data-xs-items="{{ $data_xs_items }}" data-arrows='true'>
    @php
        $top_brands_product_limit = 1;
    @endphp
    @foreach ($top_brands_products as $key => $top_brands_product)
        <div class="carousel-box top_brands_products @if ($key == $top_brands2[0]) active @endif"
             onclick="top_brands_products({{ $key }}, this)">
            <div class="size-80px border border-dashed rounded-2 overflow-hidden p-1">
                <div class="h-100 rounded-2 overflow-hidden d-flex align-items-center">
                    <img src="{{ get_file_by_id($top_brands_product[0]->logo) }}" alt="{{ translate('brands') }}"
