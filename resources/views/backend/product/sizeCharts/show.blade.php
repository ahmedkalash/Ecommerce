@if ($size_chart->fit_type != null)
    ...
    <p class="fs-14 fw-600 text-left">
        ...
        <span class="fw-400 ml-3">
            @if ($size_chart->fit_type == 'slim_fit')
                {{ translate('Slim Fit') }}
            @endif
            @if ($size_chart->fit_type == 'regular_fit')
                {{ translate('Regular Fit') }}
            @endif
            @if ($size_chart->fit_type == 'relaxed')
                {{ translate('Relaxed') }}
            @endif
        </span>
    </p>
...
@if ($size_chart->stretch_type != null)
    ...
    <p class="fs-14 fw-600 text-left">
        ...
        <span class="fw-400 ml-3">
            @if ($size_chart->stretch_type == 'non')
                {{ translate('Non') }}
            @endif
            @if ($size_chart->stretch_type == 'slight')
                {{ translate('Slight') }}
            @endif
            @if ($size_chart->stretch_type == 'medium')
                {{ translate('Medium') }}
            @endif
            @if ($size_chart->stretch_type == 'hign')
                {{ translate('Hign') }}
            @endif
        </span>
    </p>
    ...
    @if($measurement_option_inch == 1 || $measurement_option_cen == 1)
        <ul class="nav nav-tabs border-0" role="tablist">
            ...
            @if($measurement_option_cen == 1)
                <li class="nav-item">
                    <a class="nav-link w-110px @if($measurement_option_inch == 0) active @endif" data-toggle="tab"
                       data-type="cen"
                    ...
                    @foreach ($measurementPoints as $measurementPoint)
                        <tr>
                            <td class="fw-600 text-left"
                                style="display: table-cell; vertical-align: middle;">{{ $measurementPoint->name }}</td>
                            @foreach ($size_options as $size_option)
...
@if ($size_chart->photos != null || $size_chart->description != null)
    ...
    <div class="row">
        @if ($size_chart->description != null)
            <div
                class="@if ($size_chart->photos != null && $size_chart->description != null) col-md-6 @else col-md-12 @endif">
                <p class="text-justify">{!! nl2br($size_chart->description) !!}</p>
            </div>
        @endif
        @if ($size_chart->photos != null)
            <div
                class="@if ($size_chart->photos != null && $size_chart->description != null) col-md-6 @else col-md-12 @endif px-4">
                @foreach (explode(",", $size_chart->photos) as $photo)
                    <img src="{{ get_file_by_id($photo) }}" class="w-100 mb-4">
                @endforeach
            </div>
        @endif
    </div>
