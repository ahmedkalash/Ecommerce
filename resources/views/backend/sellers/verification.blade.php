        ...
        <div class="card-header">
            ...
            @if ($shop->verification_status != 1 && $shop->verification_info != null)
                <div class="pull-right clearfix">
                    <a href="{{ route('sellers.reject', $shop->id) }}"
                       class="btn btn-circle btn-danger d-innline-block">{{translate('Reject')}}</a></li>
                    <a href="{{ route('sellers.approve', $shop->id) }}"
                       class="btn btn-circle btn-success d-innline-block">{{translate('Accept')}}</a>
                </div>
            @endif
        </div>
            ...
            <div class="col-md-5">
                        ...
                        @foreach (json_decode($shop->verification_info) as $key => $info)
                            <tr>
                                ...
                                @elseif ($info->type == 'file')
                                    <td>
                                        <a href="{{ get_file_by_path($info->value) }}" target="_blank"
                ...
                @if ($shop->verification_status != 1 && $shop->verification_info != null)
                    <div class="text-center">
                        ...
                        <a href="{{ route('sellers.approve', $shop->id) }}"
                           class="btn btn-sm btn-success d-innline-block">{{translate('Accept')}}</a>
                    </div>
                @endif
            </div>
        </div>
