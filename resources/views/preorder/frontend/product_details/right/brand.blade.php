        ...
        <div class="ml-3 d-flex" style="margin-bottom: -15px !important">
            <div class="mt-2">
                <a href="{{ route('products.brand', $product->brand->slug) }}"
                   class=" avatar-md mr-2 overflow-hidden  float-left float-lg-none float-xl-left">
                    <img class="lazyload h-100 w-100 preorder-brand-image"
                         src="{{ static_asset('assets/img/placeholder.jpg') }}"
                         data-src="{{ get_file_by_id($product->brand->logo) }}"
                         onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
                </a>
            </div>
            <div class="mt-2 ml-3">
                <div> {{translate('Brand')}} <b>{{ $product->brand->name }}</b></div>
                <div class="preorder-text-secondary"><a
                        href="{{route('products.brand', $product->brand->slug)}}">{{ translate('Products from this brand') }}</a>
                </div>
            </div>
        </div>
    </div>
