                                        ...
                                        @if ($authUserId == $conversation->sender_id)
                                            @if ($conversation->receiver->shop != null)
                                                <a href="{{ route('shop.visit', $conversation->receiver->shop->slug) }}"
                                                   class="">
                                                    <img src="{{ get_file_by_id($conversation->receiver->shop->logo) }}"
                                                         onerror="this.onerror=null;this.src='{{ static_asset('assets/img/avatar-place.png') }}';">
                                                </a>
                                            @else
                                                <img
                                                    @if ($conversation->receiver->avatar_original == null) src="{{ static_asset('assets/img/avatar-place.png') }}"
                                                    @else src="{{ get_file_by_id($conversation->receiver->avatar_original) }}"
                                                    @endif
                                                    onerror="this.onerror=null;this.src='{{ static_asset('assets/img/avatar-place.png') }}';">
                                            @endif
                                        @else
                                            <img
                                                ...
                                                @else src="{{ get_file_by_id($conversation->sender->avatar_original) }}"
                                ...
                                <div class="block-body">
                                    <div class="block-body-inner pb-3">
                                        ...
                                        <div class="row no-gutters">
                                            <div class="col">
                                                <h6 class="mt-0">
                                                    <a href="{{ route('preorder-conversations.customer-show', encrypt($conversation->id)) }}"
