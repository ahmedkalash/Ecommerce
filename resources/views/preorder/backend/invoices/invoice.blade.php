...
<div>
    ...
    <div style="background: #eceff4;padding: 1rem;">
        <table>
            <tr>
                <td>
                    @if($logo != null)
                        <img src="{{ get_file_by_id($logo) }}" height="30" style="display:inline-block;">
                    @else
        ...
        <table>
            ...
            <tr>
                ...
                <td class="text-right small"><span class="gry-color small">{{  translate('Order ID') }}:</span> <span
                        class="strong">{{ $order->order_code }}</span></td>
            </tr>
            <tr>
    ...
    <div style="padding: 1rem;padding-bottom: 0">
        <table>
            ...
            <tr>
                <td class="gry-color small">{{ $order->address?->address }}, {{ $order->address?->city?->name }}
                    , {{ $order->address?->state->name }} , {{ $order->address?->country->name }}</td>
            </tr>
            <tr>
                <td class="gry-color small">{{ translate('Email') }}
                    : {{ $order->address?->email ?? $order->user->email}}</td>
            </tr>
            <tr>
                <td class="gry-color small">{{ translate('Phone') }}: {{ $order->address?->phone }}</td>
            </tr>
        </table>
    </div>
