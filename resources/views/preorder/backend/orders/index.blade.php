                        ...
                        <div class="col-12 mt-4">
                            <div class="badges ">
                                ...
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'all' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('all')">
                                    {{translate('All') }} ({{ $preorder_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'requested' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('requested')">
                                    {{ translate('Requests')}} ({{ $preorder_request_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'accepted_requests' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('accepted_requests')">
                                    {{ translate('Accepted Requests')}} ({{ $accepted_request_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'prepayment_requests' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('prepayment_requests')">
                                    {{ translate('Prepayment Requests')}} ({{ $prepayment_request_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'confirmed_prepayments' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('confirmed_prepayments')">
                                    {{ translate('Confirmed Prepayments')}} ({{ $confirmed_prepayment_request_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'final_preorders' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('final_preorders')">
                                    {{ translate('Final Preorders')}} ({{ $final_preorder_request_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'in_shipping' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('in_shipping')">
                                    {{ translate('In Shipping')}} ({{ $preorder_request_in_shipping_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'delivered' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('delivered')">
                                    {{ translate('Delivered') }} ({{ $preorder_product_delivered_count }})
                                </a>
                                <a href="javascript:void(0);"
                                   class="badge badge-inline {{ $status == 'refund' ? $activeClasss : $inActiveClasses}}"
                                   onclick="sort_order_by_status('refund')">
                    ...
                    <div class="col-9">
                        <div class="row">
                            ...
                            <div class="col-auto">
                                <div class="form-group mb-0">
                                    <button type="submit"
                                            class="btn btn-sm btn-soft-primary text-primary">{{ translate('Filter')}}</button>
                                </div>
                            </div>
                                ...
                                <select class="form-control form-control-sm aiz-selectpicker" id="bulk_action">
                                    ...
                                    @if((($routeName == 'delayed_prepayment_preorders.list' && auth()->user()->can('delayed_prepayment_preorder_notification_send')) ||
                                        ($routeName == 'delayed_final_orders.list' && auth()->user()->can('final_preorder_notification_send'))) &&
                                            $canSendNotification)
                                        <option
                                            value="prepayment_final_preorder_reminder">{{ translate('Send Notification') }}</option>
                                    @endif
                                </select>
                            </div>
                            <div class="col-auto">
                                <div class="form-group mb-0">
                                    <button type="button" onclick="bulkAction()"
                                            class="btn btn-sm btn-soft-primary text-primary">{{ translate('Apply')}}</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <tr>
                            ...
                            <td class="pl-0" style="vertical-align: middle">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="ml-2">
                                        <span
                                            class="text-muted text-truncate-1">{{ Str::limit($order->preorder_product?->getTranslation('product_name'), 50, ' ...') }}</span>
                                        <br>
                                        <span
                                            class="opacity-60 text-muted text-truncate-1">{{ translate('QTY') }} : {{ $order->quantity }}</span>
                                    </div>
                                </div>
                            </td>
                            <td>
                                ...
                                <br>
                                <span
                                    class="opacity-60 text-muted text-truncate-1">{{ translate('Created') }} : {{ $order->created_at }}</span>
                            </td>
                            <td>{{ single_price($order->grand_total) }}
                                / {{ single_price($order->preorder_product?->is_prepayment ? $order->preorder_product->preorder_prepayment?->prepayment_amount : 0)}}</td>
                            @if($routeName != 'inhouse_preorder.list')
                                <td>
                            ...
                            <td>
                                @if($order->user != null)
                                    ...
                                    <br>
                                    <span
                                        class="opacity-60 text-muted text-truncate-1">{{ $order->user->email ?? $order->user->phone }}</span>
                                @else
                            ...
                            <td>
                                @if($order->refund_status == 2)
                                    <span
                                        class="badge badge-inline badge-success m-2 p-2 rounded-3">{{ translate('Refunded')}}</span>
                                @elseif($order->delivery_status == 2)
                                    <span
                                        class="badge badge-inline badge-success m-2 p-2 rounded-3">{{ translate('Delivered')}}</span>
                                @elseif($order->shipping_status == 2)
                                    <span
                                        class="badge badge-inline badge-info p-2 m-2 rounded-3">{{ translate('In Shipping')}}</span>
                                @elseif($order->final_order_status == 1)
                                    <span
                                        class="badge badge-inline badge-warning p-2 m-2 rounded-3">{{ translate('Final Order Requested')}}</span>
                                @elseif($order->final_order_status == 2)
                                    <span
                                        class="badge badge-inline badge-success p-2 m-2 rounded-3">{{ translate('Final Order Accepted')}}</span>
                                @elseif($order->final_order_status == 3)
                                    <span
                                        class="badge badge-inline badge-danger p-2 m-2 rounded-3">{{ translate('Final Order Cancelled')}}</span>
                                @elseif($order->prepayment_confirm_status == 1)
                                    <span
                                        class="badge badge-inline badge-primary p-2 m-2 rounded-3">{{ translate('Prepayment Requested')}}</span>
                                @elseif($order->prepayment_confirm_status == 2)
                                    <span
                                        class="badge badge-inline badge-dodger-blue p-2 m-2 rounded-3">{{ translate('Prepayment Accepted')}}</span>
                                @elseif($order->prepayment_confirm_status == 3)
                                    <span
                                        class="badge badge-inline badge-dodger-blue p-2 m-2 rounded-3">{{ translate('Prepayment Cancelled')}}</span>
                                @elseif($order->request_preorder_status == 1)
                                    <span
                                        class="badge badge-inline badge-secondary p-2 m-2 rounded-3">{{ translate('Preorder Requested')}}</span>
                                @elseif($order->request_preorder_status == 2)
                                    <span
                                        class="badge badge-inline badge-gray p-2 m-2 rounded-3">{{ translate('Preorder Request Accepted')}}</span>
                                @elseif($order->request_preorder_status == 2)
                                    <span
                                        class="badge badge-inline badge-danger p-2 m-2 rounded-3">{{ translate('Preorder Request Cancelled')}}</span>
                                @endif
                            </td>
                            @if (addon_is_activated('refund_request'))
                                <td>
                                    @if($order->preorder_product?->is_refundable == 1)
                                        <span
                                            class="badge badge-inline badge-success m-2 p-2 rounded-3">{{ translate('Refundable')}}</span>
                                    @else
                                        <span
                                            class="badge badge-inline badge-soft-primary m-2 p-2 rounded-3">{{ translate('No Refund')}}</span>
                                    @endif
                                </td>
                            @endif
                            <td class="text-right">
                                @can('view_preorder_details')
                                    <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                       href="{{ route('preorder-order.show', encrypt($order->id)) }}"
                                ...
                                @if((($routeName == 'delayed_prepayment_preorders.list' && auth()->user()->can('delayed_prepayment_preorder_notification_send')) ||
                                    ...
                                    <a class="btn btn-soft-warning btn-icon btn-circle btn-sm"
                                       ...
                                       onclick="single_prepayment_final_preorder_reminder('{{ $order->id }}');"
                                ...
                                @can('delete_preorder')
                                    <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                       data-href="{{ route('preorder-order.destroy', $order->id) }}"
    ...
    <div id="prepayment_final_preorder_reminder_modal" class="modal fade">
        <div class="modal-dialog modal-md modal-dialog-centered" style="max-width: 540px;">
            <div class="modal-content pb-2rem px-2rem">
                ...
                <form class="form-horizontal" action="{{ route('prepayment_final_preorder_reminder') }}" method="POST"
                      enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body text-center">
                        <input type="hidden" name="order_ids" value="" id="order_ids">
                        <input type="hidden" name="reminder_type"
                               value="{{ ($routeName == 'delayed_prepayment_preorders.list') ?  'preorder_prepayment_reminder_customer' : 'preorder_final_order_reminder_customer' }}"
                               id="order_ids">
...
@section('script')
    <script type="text/javascript">
        ...
        function bulk_delete() {
            ...
            $.post('{{ route('bulk-preorder-delete') }}', {
                _token: '{{ csrf_token() }}',
                order_ids: orderIds
            }, function (data) {
                if (data == 1) {
