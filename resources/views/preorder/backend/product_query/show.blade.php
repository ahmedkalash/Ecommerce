            ...
            <div class="card-body">
                    ...
                    <li class="list-group-item px-0">
                        <div class="media mb-2">
                            <img class="avatar avatar-xs mr-3"
                                 @if ($query->user != null) src="{{ get_file_by_id($query->user->avatar_original) }}"
                                 @endif
                ...
                @if ((auth()->user()->id == $query->seller_id || auth()->user()->user_type == 'staff') && auth()->user()->can('reply_preorder_product_queries'))
                    <form action="{{ route('preorder.product_query.reply', $query->id) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <textarea class="form-control" rows="4" name="reply"
                        ...
                        <div class="text-right">
                            <button type="submit"
                                    class="btn btn-info">{{  $query->reply == null ? translate('Send') : translate('Update') }}</button>
                        </div>
                    </form>
                @endif
            </div>
