...
@section('meta')
    ...
    <meta itemprop="image" content="{{ get_file_by_id($shop->logo) }}">
    ...
    <meta name="twitter:image" content="{{ get_file_by_id($shop->meta_img) }}">

    <!-- Open Graph data -->
    <meta property="og:title" content="{{ $shop->meta_title }}"/>
    <meta property="og:type" content="website"/>
    <meta property="og:url" content="{{ route('shop.visit', $shop->slug) }}"/>
    <meta property="og:image" content="{{ get_file_by_id($shop->logo) }}"/>
            ...
            <section class="h-160px h-md-200px h-lg-300px h-xl-100 w-100">
                <a href="{{ $shop->top_banner_link }}">
                    <img class="d-block lazyload h-100 img-fit"
                         ...
                         data-src="{{ get_file_by_id($shop->top_banner_image) }}" alt="{{ env('APP_NAME') }} offer">
                </a>
            </section>
        @endif
    @endif
                    ...
                    <div class="col-lg-5 col-md-6">
                        <div class="d-flex align-items-center">
                            ...
                            <a href="{{ route('shop.visit', $shop->slug) }}"
                               class="overflow-hidden size-64px rounded-content" style="border: 1px solid #e5e5e5;
                                box-shadow: 0px 10px 20px rgba(0, 0, 0, 0.06);min-width: fit-content;">
                                <img class="lazyload h-64px  mx-auto"
                                     src="{{ static_asset('assets/img/placeholder.jpg') }}"
                                     data-src="{{ get_file_by_id($shop->logo) }}"
                            ...
                            <div class="d-flex justify-content-md-end pl-lg-3 pt-3 pt-lg-0">
                                ...
                                @if(in_array($shop->id, $followed_sellers))
                                    <a href="{{ route("followed_seller.remove", ['id'=>$shop->id]) }}"
                                       data-toggle="tooltip" data-title="{{ translate('Unfollow Seller') }}"
                                       data-placement="top"
                                       ...
                                       style="height: 40px; border-radius: 30px !important; justify-content: center;">
                                        ...
                                        <span class="fw-700">{{ translate('Followed') }}</span> &nbsp;
                                        ({{ $shopFollowers }})
                                    </a>
                                @else
                                    <a href="{{ route("followed_seller.store", ['id'=>$shop->id]) }}"
                                       ...
                                       style="height: 40px; border-radius: 30px !important; justify-content: center;">
                                        ...
                                        <span class="fw-700">{{ translate('Follow Seller') }}</span> &nbsp;
                                        ({{ $shopFollowers }})
                                    </a>
                                @endif
                            </div>
    ...
    @if (!isset($type))
        ...
        @if (count($feature_products) > 0)
            <section class="mt-3 mb-3" id="section_featured">
                <div class="container">
                    ...
                    <!-- Products Section -->
                    <div class="px-sm-3">
                        <div class="aiz-carousel sm-gutters-16 arrow-none featured-products" data-items="6"
                             data-xl-items="5" data-lg-items="4" data-md-items="3" data-sm-items="2.5"
        ...
        @if ($shop->slider_images != null)
            <section class="mt-3 mb-3">
                <div class="container">
                    <div class="aiz-carousel mobile-img-auto-height" data-arrows="true" data-dots="false"
                         data-autoplay="true">
                        @php
                        ...
                        @foreach ($shop_slider_images as $key => $slider)
                            <div class="carousel-box w-100 h-140px h-md-300px h-xl-450px">
                                <a href="{{ isset($shop_slider_links[$key]) ? $shop_slider_links[$key] : '' }}">
                                    <img class="d-block lazyload h-100 img-fit"
                                         src="{{ $slider ? get_file_by_path($slider->file_name) : static_asset('assets/img/placeholder.jpg') }}"
        ...
        @if (count($coupons)>0)
            <section class="mt-3 mb-3" id="section_coupons">
                <div class="container">
                    ...
                    <!-- Coupons Section -->
                    <div class="aiz-carousel sm-gutters-16 arrow-none" data-items="3" data-lg-items="2"
                         data-sm-items="1" data-arrows='true' data-infinite='false'>
                ...
                <section class="container mb-3 mt-3">
                    <div class="w-100">
                        <a href="{{ isset($shop_banner_full_width_1_links[$key]) ? $shop_banner_full_width_1_links[$key] : '' }}">
                            <img class="d-block lazyload h-100 img-fit"
                                 src="{{ $banner ? get_file_by_path($banner->file_name) : static_asset('assets/img/placeholder.jpg') }}"
                                 onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.jpg') }}';"
        ...
        <div class="container">
            ...
            @if (!isset($type))
                ...
                <div class="px-sm-3 pb-3">
                    <div class="aiz-carousel sm-gutters-16 arrow-none featured-products" data-items="6"
                         data-xl-items="5" data-lg-items="4" data-md-items="3" data-sm-items="2.5" data-xs-items="2.5"
