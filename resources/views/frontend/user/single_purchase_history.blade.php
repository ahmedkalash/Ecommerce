        ...
        <div class="d-flex justify-content-between align-items-center mb-2">
            <div>
                <p class="mb-2 fs-16 fw-700 deep-blue mb-0"><a class="deep-blue"
                        ...
                        - {{ $order->code }}</a></p>
                <span
                    class="text-muted d-block d-md-none fs-12">{{ translate('Date')}}: {{ date('d-m-Y', $order->date) }}</span>
            </div>
                        ...
                        @if ($order->delivery_status == 'pending' && $order->payment_status == 'unpaid')
                            <a href="javascript:void(0)"
                               ...
                               data-href="{{route('purchase_history.destroy', $order->id)}}"><i
                                    class="las la-trash mr-2"></i> {{ translate('Cancel') }}</a>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <div class="row">
                        <div class="col-md-8 d-flex align-items-center">
                            ...
                            <div class="w-300px text-wrap">
                                <div
                                    class="font-weight-semibold fs-14 product-name-color mobile-title-shift text-truncate-2"
                                    title="{{ $orderDetail->product->getTranslation('name') }}">
                                    {{ $orderDetail->product->getTranslation('name') }}
                                </div>
                                <div
                                    class="text-muted small mb-2 mobile-title-shift">{{ $orderDetail->variation }}</div>
                            </div>
