<script>
    function account_delete_confirm_modal() {
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header d-block py-4">
                ...
                <p class="fs-16 fw-600 text-center"
                   style="color: #8d8d8d;">{{ translate('Warning: You cannot undo this action')}}</p>
            </div>

            <form action="{{ route('account_delete') }}" method="POST">
                @csrf
                <div class="modal-body pt-3 pb-5 px-xl-5">
                    <p class="text-danger mt-3 fw-800"><i>{{ translate('Note: ')}}
                            &nbsp;{{ translate("Don't Click to any button or don't do any action during account Deletion, it may takes some times.")}}</i>
                    </p>
                    ...
                    <div class="mt-4">
                        <label class="fs-14 fw-700"
                               style="color: #8d8d8d;">{{ translate('Enter your password to confirm:') }}</label>
                        <input type="password"
                               name="password"
                               class="form-control @error('password') is-invalid @enderror"
                               placeholder="{{ translate('Your current password') }}"
                               required>
                        @error('password')
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit"
                            class="btn btn-danger rounded-0 w-150px">{{ translate('Delete Account')}}</button>
                </div>
            </form>
        </div>
    </div>
