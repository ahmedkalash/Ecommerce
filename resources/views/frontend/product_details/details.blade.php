<div class="text-left">
    ...
    @if ($detailedProduct->has_warranty == 1 && $detailedProduct->warranty_id != null)
        <div class="d-flex flex-wrap align-items-center mb-3">
            ...
            <img src="{{ get_file_by_id($detailedProduct->warranty->logo) }}" height="40">
    ...
    @if ($detailedProduct->auction_product != 1)
        <form id="option-choice-form">
            ...
            @if ($detailedProduct->digital == 0)
                ...
                <!-- Choice Options -->
                @php
                    $choice_options = is_string($detailedProduct->choice_options) ? json_decode($detailedProduct->choice_options) : $detailedProduct->choice_options;
                @endphp
                ...
                <!-- Color Options -->
                @php
                    $colors = is_string($detailedProduct->colors) ? json_decode($detailedProduct->colors) : $detailedProduct->colors;
                @endphp
                        ...
                        <div class="product-quantity d-flex align-items-center">
                            <div class="row no-gutters align-items-center aiz-plus-minus mr-3" style="width: 130px;">
                                ...
                                <input type="number" name="quantity"
                                       ...
                                       value="{{ $detailedProduct->stocks->min('min_qty') ?? 1 }}"
                                       min="{{ $detailedProduct->stocks->min('min_qty') ?? 1 }}"
                                       max="100" lang="en">
                                <button class="btn col-auto btn-icon btn-sm btn-light rounded-0" type="button"
                            ...
                            <div class="avialable-amount opacity-60">
                                @if (true)
                                    {{-- stock_visibility_state dropped, always show if qty --}}
            ...
            @if ($detailedProduct->digital == 0)
                ...
                @else
                    ...
                    <button type="button" class="btn btn-primary mr-2 buy-now fw-600 add-to-cart min-w-150px rounded-0"
                            @if (Auth::check() || get_Setting('guest_checkout_activation')==1) onclick="buyNow()"
                            @else onclick="showLoginModal()" @endif>
