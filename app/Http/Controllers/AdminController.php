...
class AdminController extends Controller
{
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        $data['cached_graph_data'] = Cache::remember('cached_graph_data', 86400, function () use ($root_categories) {
            ...
            foreach ($root_categories as $key => $category) {
                ...
                foreach ($products as $key => $product) {
                    // $sale += $product->num_of_sale; // Column removed
                    // $sale += $product->orderDetails()->sum('quantity'); // Too slow for loop
                    $sale += 0; // Placeholder until efficient aggregation is implemented
        ...
        $data['total_customers'] = User::where('user_type', UserType::CUSTOMER->value)->where(
            'email_verified_at',
            '!=',
            null
        )->count();
        $data['top_customers'] = User::select(
            'users.id',
            'users.name',
            ...
            ->where('users.user_type', UserType::CUSTOMER->value)
            ->orderBy('total', 'desc')
            ->limit(6)
            ->get();
        $data['total_products'] = Product::where('approved', 1)->where('published', 1)->count();
        $data['total_inhouse_products'] = Product::where('approved', 1)->where('published', 1)->where(
            'added_by',
            UserType::ADMIN->value
        )->count();
        $data['total_sellers_products'] = Product::where('approved', 1)->where('published', 1)->where(
            'added_by',
            '!=',
            UserType::ADMIN->value
        )->count();
        ...
        $data['total_sellers'] = User::where('user_type', UserType::SELLER->value)->where('email_verified_at', '!=',
            null)->count();
        $data['status_wise_sellers'] = Shop::select('verification_status', DB::raw('COUNT(*) as total'))
            ->whereIn('user_id', function ($q) {
                $q->select('id')
                    ->from(with(new User)->getTable())
                    ->where('user_type', UserType::SELLER->value)
        ...
        $admin = User::where('user_type', UserType::ADMIN->value)->first();
        ...
        $data['payment_type_wise_inhouse_sale'] = Order::select(DB::raw('case
            ...
            ->groupBy(DB::raw('1'))
            ->get();
        $data['inhouse_product_rating'] = Product::where('added_by', UserType::ADMIN->value)->where('rating', '!=',
            0)->avg('rating');
    ...
    public function inhouse_top_categories(Request $request)
    {
        ...
        $inhouse_top_category_query->select(
            'categories.id',
            'categories.name',
            'categories.cover_image',
    ...
    public function inhouse_top_brands(Request $request)
    {
        ...
        $inhouse_top_brand_query->select(
            'brands.id',
            'brands.name',
            'brands.logo',
            DB::raw('SUM(order_details.price + order_details.tax) as total')
    ...
    {
        $top_brands_products = DB::table(DB::raw('(SELECT products.id product_id, products.name product_name, products.slug product_slug, 0 as auction_product, products.brand_id,
