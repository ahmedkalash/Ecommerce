    ...
    public function getList(Request $request)
    {
        ...
        if ($list == 'online' || $list == 'both') {
            ...
            if (count($all_online_payment_methods) > 0) {
                $available_online_payment_methods = [
                    'paypal', 'stripe', 'instamojo', 'razorpay', 'paystack', 'iyzico', 'bkash', 'nagad', 'sslcommerz',
                    'aamarpay', 'flutterwave', 'payfast', 'paytm', 'khalti', /* "myfatoorah", */
                    'phonepe', /* "cybersource" */
                ];
                if (get_setting('phonepe_version', '1') != 2) {
                    $available_online_payment_methods = array_diff($available_online_payment_methods, ['phonepe']);
                }
                $online_payment_methods = $all_online_payment_methods->toQuery()->whereIn('name',
                    $available_online_payment_methods)->get();
        ...
        if ($mode != 'wallet' && $mode != 'seller_package' && $list != 'offline') {
            ...
            if ($mode == 'order') {
                ...
                $carts = ($user != null) ?
                    ...
                    ($request->has('temp_user_id') ? Cart::where('temp_user_id',
                        $request->temp_user_id)->active()->get() : []);
        ...
        if (($list == 'offline' || $list == 'both') && addon_is_activated('offline_payment')) {
            foreach (ManualPaymentMethod::all() as $method) {
                ...
                $payment_type['image'] = get_file_by_id($method->photo);
