...
class ChatController extends Controller
{
    ...
    {
        $messages = Message::where('conversation_id', $conversation_id)->where('id', '>',
            $last_message_id)->latest('id')->paginate(10);
    ...
    public function create_conversation(Request $request)
    {
        ...
        return response()->json([
            'result' => true, 'conversation_id' => $conversation->id,
            'shop_name' => $conversation->receiver->user_type == 'admin' ? 'In House Product' : $conversation->receiver->shop->name,
            'shop_logo' => $conversation->receiver->user_type == 'admin' ? get_file_by_id(get_setting('header_logo')) : get_file_by_id($conversation->receiver->shop->logo),
