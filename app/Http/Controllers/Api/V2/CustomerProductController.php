...
class CustomerProductController extends Controller
{
    ...
    public function relatedProducts($slug)
    {
        ...
        $products = CustomerProduct::where('category_id', $product->category_id)->where('id', '!=',
            $product->id)->where('status', '1')->where('published', '1')->paginate(10);
    ...
    public function store(Request $request)
    {
        ...
        if ($customer_product->save()) {
            ...
            $customer_product_translation = CustomerProductTranslation::firstOrNew([
                'lang' => env('DEFAULT_LANGUAGE'), 'customer_product_id' => $customer_product->id,
            ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($customer_product->save()) {
            ...
            $customer_product_translation = CustomerProductTranslation::firstOrNew([
                'lang' => $request->lang, 'customer_product_id' => $customer_product->id,
            ]);
            $customer_product_translation->name = $request->name;
            $customer_product_translation->unit = $request->unit;
            $customer_product_translation->description = $request->description;
